...
static void fed_drain_cb(const V4xCommitEntry *e, void *ud)
{
    ...
    /* feed v4x output into federation write path
     * v_snapped = canonical grid value (use as packed proxy)
     * v_clean   = angular addr (PHI-scattered)
     * value     = v_snapped as data payload                           */
    fed_write(fed,
              (uint32_t)(e->v_snapped & 0xFFFFFFFu),  /* packed (no iso) */
              (uint64_t) e->v_clean,                   /* angular_addr    */
              (uint64_t) e->v_snapped);                /* value           */
