...
#ifndef CORE_C_POGLS_DELTA_WORLD_B_H
#define CORE_C_POGLS_DELTA_WORLD_B_H

/* Include core_c/pogls_delta.h — handles guard CORE_C_POGLS_DELTA_H
 * If storage/pogls_delta.h was included first (guard POGLS_DELTA_H),
 * core_c types must be re-declared here explicitly.              */
#include <stdbool.h>
#ifndef CORE_C_POGLS_DELTA_H
#  include "pogls_delta.h"  /* resolves to core_c/pogls_delta.h via -Icore_c */
#endif
/* Ensure LANE_COUNT is available (core_c/pogls_delta.h defines it as 4) */
#ifndef LANE_COUNT
#  define LANE_COUNT 4u
#endif
/* Ensure Delta_Context is available — re-declare if core_c guard missed */
/* Delta_Context defined in core_c/pogls_delta.h */
...
#define LANE_B_COUNT         4
#define LANE_TOTAL           8   /* World A(4) + World B(4) */
...
typedef struct __attribute__((packed)) {
    ...
    uint64_t seq_b[LANE_B_COUNT];/* World B lane seqs [4-7] */
    ...
    uint32_t crc32;              /* CRC32(bytes 0..139) */
} Delta_DualMerkleRecord;
/* sizeof = 4+4+8+(8×4)+(8×4)+32+32+32+4 = 180 bytes */
...
typedef struct {
    ...
    int      lane_fd[LANE_B_COUNT];    /* fd ของ .pending files (index 0-3 = B lanes) */
...
#endif /* CORE_C_POGLS_DELTA_WORLD_B_H */
